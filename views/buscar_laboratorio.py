...
# Índices de columna en la fila devuelta por listar_laboratorios().
# Deben coincidir EXACTAMENTE con el orden del SELECT en
# database/laboratorio/buscar_laboratorio.py -> listar_laboratorios().
#
# id(0) fecha_practica(1) carrera(2) laboratorio(3) asignatura(4)
# docente_responsable(5) tema_practica(6) pdf_url(7)
...
def _fecha_dt(fila) -> datetime | None:
    """Devuelve el datetime de la práctica para comparaciones de rango."""
    val = fila[IDX_FECHA]
    if val is None:
        return None
    if isinstance(val, datetime):
...
class VentanaBuscarLaboratorio(ctk.CTkToplevel):

    def __init__(self, master):
        super().__init__(master)

        self.title("Buscar Laboratorios")
    ...
    def _aplicar_filtros(self):
        ...
        for f in self._todos:
            ...
            if q and not any(
                q in str(f[i]).lower()
                for i in (IDX_CARRERA, IDX_LAB, IDX_ASIGN, IDX_DOCENTE, IDX_TEMA)
            ):
                continue
    ...
    def _renderizar(self, datos):
        ...
        self._lbl_count.configure(
            text=f"{total} laboratorio{'s' if total != 1 else ''} encontrado{'s' if total != 1 else ''}"
        )

        if not datos:
            ctk.CTkLabel(
                self.frame,
                text="No se encontraron laboratorios con esos criterios.",
                font=("Consolas", 13),
                text_color=TEXT_SEC,
            ).pack(pady=40)
            return
    ...
    def _fila_card(self, fila):
        ...
        # Fila superior: FECHA destacada + carrera + laboratorio + asignatura
        ...
        _btn(btn_row, "✏ Editar",   BG_PANEL, BG_CARD_HOV, TEXT_PRI,
             lambda i=id_lab: self.editar(i))
        _btn(btn_row, "🗑 Eliminar", RED,      RED_DARK,    TEXT_PRI,
             lambda i=id_lab: self.eliminar_y_recargar(i))
    ...
    def editar(self, id_lab):
        fila = buscar_laboratorio_por_id(id_lab)
        if not fila:
            messagebox.showerror("Error", "No se encontró el laboratorio.")
            return

        materiales  = buscar_materiales_por_laboratorio(id_lab)
        reactivos   = buscar_reactivos_por_laboratorio(id_lab)
        estudiantes = buscar_estudiantes_por_laboratorio(id_lab)

        registro = Laboratorio.from_row(
            fila,
            materiales=materiales,
            reactivos=reactivos,
            estudiantes=estudiantes,
        )
        VentanaEditarLaboratorio(self, registro)

    def eliminar_y_recargar(self, id_lab):
        if not messagebox.askyesno("Confirmar", "¿Desea eliminar este laboratorio?"):
            return
        exito = eliminar(id_lab)
        if not exito:
            messagebox.showerror("Error", "No fue posible eliminar el laboratorio.")
            return
        messagebox.showinfo("Correcto", "Laboratorio eliminado correctamente.")
        self.cargar_datos()