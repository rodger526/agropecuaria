...
# tipo_practica(6) lugar_ejecucion(7) semana_planificada(8)
# tema_practica(9) resultado_aprendizaje(10)
# articulacion_curricular(11) objetivo_general(12)
# materiales_equipos(13) descripcion_actividad(14) evidencias(15)
# pdf_url(16) fecha_registro(17) ingeniero_revisor(18)
# fecha_creacion(19) firma_docente(20) firma_comision(21)
...
def _col(registro, idx, default=""):
    """
    Obtiene de forma segura una columna de la tupla devuelta
...
class VentanaEditarPractica(ctk.CTkToplevel):
    ...
    def __init__(self, master, registro):
        ...
        # Datos originales que no se editan en este formulario,
        # pero se requieren para regenerar correctamente el PDF.
        self._fecha_creacion_original = _col(
            registro,
            IDX_FECHA_CREACION,
            None,
        )

        self._firma_docente_bd = _col(
            registro,
            IDX_FIRMA_DOC,
            None,
        )

        self._firma_comision_bd = _col(
            registro,
            IDX_FIRMA_COM,
            None,
        )
        ...
        self.semestre.insert(
            0,
            str(_col(registro, IDX_SEMESTRE)),
        )
        ...
        self.evidencias.insert(
            0,
            _col(registro, IDX_EVIDENCIAS),
        )
        self.evidencias.pack(fill="x", pady=5)

        # ══ SECCIÓN 4 — ESTADO DE FIRMAS ═════════════════════════════
        firma_doc_existente = bool(
            _col(registro, IDX_FIRMA_DOC, None)
        )

        firma_com_existente = bool(
            self._firma_comision_bd
        )
        ...
        estado_row = ctk.CTkFrame(
            s4,
            fg_color="transparent",
        )
        estado_row.pack(fill="x", pady=(0, 10))

        # Docente: solo informativo.
    ...
    def _iniciar_servidor_comision(self):
        ...
        self._btn_qr_com.configure(
            text="↺  Regenerar QR"
        )

    def _generar_qr_comision(self):
        try:
            conexion_socket = socket.socket(
                socket.AF_INET,
                socket.SOCK_DGRAM,
            )

            conexion_socket.connect(
                ("8.8.8.8", 80)
            )

            ip = conexion_socket.getsockname()[0]
    ...
    def _polling_firma_comision(self):
        ...
        ruta_comision = "firma/firmas/firma_comision.png"

        if os.path.exists(ruta_comision):
            self._lbl_estado_com.configure(
                text="✔  Firmado",
                text_color=ACCENT,
            )

            self._mostrar_preview_comision(
                ruta_comision
            )

            # Guardar la ruta en la base de datos inmediatamente.
            actualizado = actualizar_firma_comision(
                self.id_practica,
                ruta_comision,
            )

            if actualizado:
                self._firma_comision_bd = ruta_comision
            else:
                messagebox.showwarning(
                    "Advertencia",
                    "La firma fue capturada, pero no fue posible "
    ...
    def _mostrar_preview_comision(self, ruta):
        ...
        try:
            ...
            # Permite manejar imágenes con y sin canal alfa.
            ...
            self._lbl_preview_com.configure(
                image=photo,
                text="",
            )
            ...
            print(
                "[editar_practica] "
                f"No se pudo mostrar la firma de comisión: {e}"
            )
    ...
    def actualizar(self):
        try:
            ...
            materiales = self.materiales.get(
                "1.0",
                "end",
            ).strip()
        ...
        # ── Actualizar los datos principales en PostgreSQL ────────────
        resultado_bd = actualizar_practica(
            ...
            evidencias,
            self.pdf_url,
            None,                     # La firma del docente no se reemplaza.
            self._firma_comision_bd,  # Mantiene o actualiza la comisión.
        )

        if not resultado_bd:
            messagebox.showerror(
                "Error",
                "No fue posible actualizar la práctica.",
            )
            return

        # ── Crear objeto actualizado para regenerar el PDF ────────────
        try:
            practica = Practica(
                carrera,
                semestre,
            ...
            # Conservar los datos originales que no se editan.
            if self._fecha_creacion_original:
                practica.fecha_creacion = (
                    self._fecha_creacion_original
                )

            practica.firma_docente = (
                self._firma_docente_bd
            )
            ...
            # Crear carpeta local para generar el PDF temporal.
            os.makedirs(
                "pdfs_planificacion",
                exist_ok=True,
            )

            # Crear un nombre estable y válido.
            nombre_pdf = (
                str(self.codigo).strip()
                if self.codigo
                else f"PRA-{self.id_practica}"
            )

            # Evitar agregar ".pdf" dos veces.
            ...
            # 1. Generar el archivo PDF local.
            generar_pdf(
                practica,
                ruta_pdf,
            )
            ...
            if not os.path.isfile(ruta_pdf):
                raise FileNotFoundError(
                    ...
                    f"{os.path.abspath(ruta_pdf)}"
                )

            # 2. Subir el archivo generado a Supabase Storage.
            nueva_url = subir_pdf(ruta_pdf)

            if not nueva_url:
                raise RuntimeError(
                    "Supabase no devolvió una URL pública "
                    "después de subir el PDF."
                )
            ...
            if not nueva_url.lower().startswith(
                ...
                raise RuntimeError(
                    "La respuesta de Supabase no contiene "
                    "una URL pública válida."
                )

            # 3. Actualizar la nueva URL en PostgreSQL.
            url_actualizada = actualizar_pdf_url(
                self.id_practica,
                nueva_url,
            )

            if not url_actualizada:
                raise RuntimeError(
                    "El PDF fue subido a Supabase, pero no se "
                    "pudo actualizar pdf_url en PostgreSQL."
                )

            # 4. Mantener la nueva URL en memoria.
            self.pdf_url = nueva_url
            practica.pdf_url = nueva_url

        except Exception as e:
            # Los datos principales ya se actualizaron.
            messagebox.showwarning(
                "Advertencia",
                "La práctica se actualizó en la base de datos,\n"
                "pero no fue posible regenerar o subir el PDF:\n\n"
                f"{e}",
            )
            return
        ...
        messagebox.showinfo(
            ...
            "El PDF fue regenerado y subido a Supabase.",
        )
